...
if (isset($_GET['table']) && $_GET['table'] == 'orders') {
    ...
    $sql = "SELECT * FROM orders " . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
if (isset($_GET['table']) && $_GET['table'] == 'orders_trans') {
    $offset = 0;
    $limit = 10;
    $where = '';
    $sort = 'date';
    ...
    $sql = "SELECT COUNT(l.id) AS total FROM `orders_trans` l " . $join;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
   
     $sql = "SELECT l.id AS id,l.*,u.name,u.mobile  FROM `orders_trans` l " . $join . " ORDER BY $sort $order LIMIT $offset, $limit";
     $db->sql($sql);
     $res = $db->getResult();
    ...
    foreach ($res as $row) {
       ...
       // $operate .= ' <a class="text text-danger" href="delete-scratch_cards.php?id=' . $row['id'] . '"><i class="fa fa-trash"></i>Delete</a>';
        ...
        $tempRow['name'] = $row['name'];
        $tempRow['mobile'] = $row['mobile'];
        $tempRow['order_count'] = $row['order_count'];
        $tempRow['start_time'] = $row['start_time'];
        $tempRow['end_time'] = $row['end_time'];
        $rows[] = $tempRow;
    }
